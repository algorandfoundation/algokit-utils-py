...
from algokit_utils.app.app import APP_PAGE_MAX_SIZE
...
if TYPE_CHECKING:
    ...
    from algokit_utils.utils.amount import AlgoAmount
    from algosdk.abi import Method
    from algosdk.box_reference import BoxReference
    from algosdk.v2client.algod import AlgodClient

ABIValue = str | int | bool | bytearray | list["ABIValue"]
...
class CommonTransactionParams:
    sender: str
    signer: TransactionSigner | TransactionSignerAccount | None = None
...
class AppMethodCall(Generic[T]):
    """
    Represents an ABI method call for Algorand applications.

    This class is a generic representation of an application method call,
    supporting different types of application parameters (Create, Update, Call).

    Attributes:
        method (ABIMethod): The ABI method to be called.
        args (Optional[List[Union[ABIValue, ...]]): Arguments for the ABI method.
        **kwargs: Additional parameters specific to the application call type.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # Assign other attributes from T, excluding 'args'
    ...
    def app_id(self) -> int | None:
        """
        Returns the application ID for this method call.
        """
        return self.params.get("app_id")

    @property
    def sender(self) -> str:
        sender = self.params.get("sender")
        if not sender and not isinstance(sender, str):
            raise ValueError("'sender' must be provided in method call parameters")
        return str(sender)

    @property
    def signer(self) -> TransactionSigner | TransactionSignerAccount | None:
        return self.params.get("signer")

    @property
    def rekey_to(self) -> str | None:
        return self.params.get("rekey_to")

    @property
    def note(self) -> bytes | None:
        return self.params.get("note")

    @property
    def lease(self) -> bytes | None:
        return self.params.get("lease")

    @property
    def static_fee(self) -> AlgoAmount | None:
        return self.params.get("static_fee")

    @property
    def extra_fee(self) -> AlgoAmount | None:
        return self.params.get("extra_fee")

    @property
    def max_fee(self) -> AlgoAmount | None:
        return self.params.get("max_fee")

    @property
    def validity_window(self) -> int | None:
        return self.params.get("validity_window")

    @property
    def first_valid_round(self) -> int | None:
        return self.params.get("first_valid_round")

    @property
    def last_valid_round(self) -> int | None:
        return self.params.get("last_valid_round")

    def __repr__(self) -> str:
        return f"AppMethodCall(method={self.method.name}, app_id={self.app_id})"


class AppCreateMethodCall(AppMethodCall[AppCreateParams]):
    pass


class AppDeleteMethodCall(AppMethodCall[AppDeleteParams]):
    pass


class AppUpdateMethodCall(AppMethodCall[AppUpdateParams]):
    pass


class AppCallMethodCall(AppMethodCall[AppMethodCallParams]):
    pass
...
class AlgoKitComposer:
    ...
    def __init__(  # noqa: PLR0913
        ...
        self.app_manager = app_manager  # TODO: implement app manager

    def add_transaction(self, transaction: Transaction, signer: TransactionSigner | None = None) -> AlgoKitComposer:
    ...
    def add_app_create_method_call(self, params: AppCreateMethodCall) -> AlgoKitComposer:
        self.txns.append(params)
        return self

    def add_app_update_method_call(self, params: AppUpdateMethodCall) -> AlgoKitComposer:
        self.txns.append(params)
        return self

    def add_app_delete_method_call(self, params: AppDeleteMethodCall) -> AlgoKitComposer:
        self.txns.append(params)
        return self

    def add_app_call_method_call(self, params: AppCallMethodCall) -> AlgoKitComposer:
        self.txns.append(params)
        return self
    ...
    def _common_txn_build_step(
        self,
        params: CommonTransactionParams | AppMethodCall,
        txn: Transaction,
    ...
    def _build_app_call(
        ...
        approval_program = params.approval_program if isinstance(params, AppUpdateParams | AppCreateParams) else None
        clear_program = params.clear_program if isinstance(params, AppUpdateParams | AppCreateParams) else None

        if isinstance(approval_program, str):
            approval_program = b""  # TODO: call appmanager.compile_teal or raw teal strs

        if isinstance(clear_program, str):
            clear_program = b""  # TODO: call appmanager.compile_teal or raw teal strs
        ...
        sdk_params = {
            ...
            "boxes": params.box_references,  # Call appmanager to get boxreferences
        ...
        if not app_id and isinstance(params, AppCreateParams):
            ...
            txn = algosdk.transaction.ApplicationCreateTxn(
                **sdk_params,
                ...
                or math.floor((len(approval_program) + len(clear_program)) / APP_PAGE_MAX_SIZE)
    ...
    def execute(
        ...
        *,
        ...
        supress_log: bool | None = None,
        populate_app_call_resources: bool | None = None,
    ) -> AtomicTransactionResponse:
        return self.send(
            max_rounds_to_wait=max_rounds_to_wait,
            supress_log=supress_log,
