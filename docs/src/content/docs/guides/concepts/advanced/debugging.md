...
The [`UpdatableConfig`](/algokit-utils-py/api/algokit_utils/config/#updatableconfig) class (source: `src/algokit_utils/config.py`) manages configuration settings for the AlgoKit project. A singleton instance is available as `config`:
...
[`AlgoKitLogger`](/algokit-utils-py/api/algokit_utils/config/#algokitlogger) is a custom `logging.Logger` subclass that provides fine-grained control over log output. It is the default logger for the library.
