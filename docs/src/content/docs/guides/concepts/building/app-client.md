...
To get an instance of `AppFactory` you can use either [`AlgorandClient`](/algokit-utils-py/guides/concepts/core/algorand-client/) via `algorand.client.get_app_factory` or instantiate it directly (passing in an app spec, an `AlgorandClient` instance and other optional parameters):
...
To get an instance of `AppClient` you can use either [`AlgorandClient`](/algokit-utils-py/guides/concepts/core/algorand-client/) via `algorand.client.get_app_client_*` or instantiate it directly (passing in an app ID, app spec, `AlgorandClient` instance and other optional parameters):
...
- `factory.send.bare.create(...)` - Signs and sends a transaction to create an app and returns a tuple of an [`AppClient`](#appclient) instance for the created app and the [result of that call](/algokit-utils-py/guides/concepts/building/app/#creation)
- `factory.deploy(...)` - Uses the [creator address and app name pattern](/algokit-utils-py/guides/concepts/building/app-deploy/#lookup-deployed-apps-by-name) to find if the app has already been deployed or not and either creates, updates or replaces that app based on the [deployment rules](/algokit-utils-py/guides/concepts/building/app-deploy/#performing-a-deployment) (i.e. it's an idempotent deployment) and returns a tuple of an [`AppClient`](#appclient) instance for the created/updated/existing app and the [result of the deployment](/algokit-utils-py/guides/concepts/building/app-deploy/#return-value)

### Create

The create method is a wrapper over the `app_create` (bare calls) and `app_create_method_call` (ABI method calls) [methods](/algokit-utils-py/guides/concepts/building/app/#creation), with the following differences:
...
If you want to construct a custom create call, use the underlying [`algorand.send.app_create` / `algorand.create_transaction.app_create` / `algorand.send.app_create_method_call` / `algorand.create_transaction.app_create_method_call` methods](/algokit-utils-py/guides/concepts/building/app/#creation) then you can get params objects:

- `factory.params.create(params)` - ABI method create call for deploy method or an underlying [`app_create_method_call` call](/algokit-utils-py/guides/concepts/building/app/#creation)
- `factory.params.bare.create(params)` - Bare create call for deploy method or an underlying [`app_create` call](/algokit-utils-py/guides/concepts/building/app/#creation)

### Deploy

The deploy method is a wrapper over the [`AppDeployer`'s `deploy` method](/algokit-utils-py/guides/concepts/building/app-deploy/#performing-a-deployment), with the following differences:
...
If you want to construct a custom deploy call, use the underlying [`algorand.app_deployer.deploy` method](/algokit-utils-py/guides/concepts/building/app-deploy/#performing-a-deployment) then you can get params objects for the `create_params`, `update_params` and `delete_params`:
...
- A `FundAppAccountParams`, which has the same properties as a [payment transaction](/algokit-utils-py/guides/concepts/building/transfer/#payment) except `receiver` is not required and `sender` is optional (if not specified then it will be set to the app client's default sender if configured).
...
When this debug flag is set, it will also emit debugging symbols to allow break-point debugging of the calls if the [project root is also configured](/algokit-utils-py/guides/concepts/advanced/debugging/).
