...
import { CATEGORIES, parseDocstring, extractOrder, createSlug } from '../src/loaders/examples-loader.ts'

const REPO_ROOT = path.resolve(import.meta.dirname, '..', '..')
const EXAMPLES_DIR = path.join(REPO_ROOT, 'examples')
const OUTPUT_DIR = path.join(REPO_ROOT, 'docs', 'src', 'content', 'docs', 'examples')
const GITHUB_BASE = 'https://github.com/algorandfoundation/algokit-utils-py/blob/main/examples'
...
for (const [categoryDir, meta] of Object.entries(CATEGORIES)) {
  ...
  for (const filename of files) {
    ...
    const runCommand = `uv run python ${categoryDir}/${filename}`
...
\`\`\`bash
cd examples
uv run python ${examples[0].categoryDir}/${examples[0].slug.replace(/-/g, '_')}.py
\`\`\`
`,
  )
}

// Top-level examples index — custom card grid
...
fs.writeFileSync(
  ...
  `---
title: Code Examples
description: "${totalCount} runnable Python examples demonstrating AlgoKit Utils features"
sidebar:
  order: 0
---

Browse **${totalCount}** runnable Python examples organized by feature area. Each example is self-contained and demonstrates specific functionality of the AlgoKit Utils library.
