...
The key function to facilitate Algo transfers is `algorand.send.payment(params)` (immediately send a single payment transaction), `algorand.create_transaction.payment(params)` (construct a payment transaction), or `algorand.new_group().add_payment(params)` (add payment to a group of transactions) per [`AlgorandClient`](/algokit-utils-py/guides/concepts/core/algorand-client/) [transaction semantics](/algokit-utils-py/guides/concepts/core/algorand-client/#creating-and-issuing-transactions).

The base type for specifying a payment transaction is `PaymentParams`, which has the following parameters in addition to the [common transaction parameters](/algokit-utils-py/guides/concepts/core/algorand-client/#transaction-parameters):
...
- The source (dispenser):
  ...
  - In `ensure_funded_from_testnet_dispenser_api`: `dispenser_client: TestNetDispenserApiClient` - a client instance of the [TestNet dispenser API](/algokit-utils-py/guides/concepts/advanced/dispenser-client/)
...
- An `options` object, which has:
  - [Common transaction parameters](/algokit-utils-py/guides/concepts/core/algorand-client/#transaction-parameters) (not for TestNet Dispenser API)
  - [Execution parameters](/algokit-utils-py/guides/concepts/core/algorand-client/#sending-a-single-transaction) (not for TestNet Dispenser API)
...
The first two variants return an `EnsureFundedResult` (which also extends the [single transaction result](/algokit-utils-py/guides/concepts/core/algorand-client/#sending-a-single-transaction)) if a funding transaction was needed, or `None` if no transaction was required. The TestNet Dispenser API variant returns an `EnsureFundedFromTestnetDispenserApiResult` or `None`. All result types share these common fields:
...
If you are using the TestNet Dispenser API then the `transaction_id` is useful if you want to use the [refund functionality](/algokit-utils-py/guides/concepts/advanced/dispenser-client/#registering-a-refund).
...
- Get a dispenser via [account manager](/algokit-utils-py/guides/concepts/core/account/#dispenser) - either automatically from [LocalNet](https://github.com/algorandfoundation/algokit-cli/blob/main/docs/features/localnet.md) or from the environment
- By programmatically creating one of the many account types via [account manager](/algokit-utils-py/guides/concepts/core/account/#accounts)
- By programmatically interacting with [KMD](/algokit-utils-py/guides/concepts/core/account/#kmd-account-management) if running against LocalNet
- By using the [AlgoKit TestNet Dispenser API client](/algokit-utils-py/guides/concepts/advanced/dispenser-client/) which can be used to fund accounts on TestNet via a dedicated API service
