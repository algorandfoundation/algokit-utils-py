...
from .reference import is_abi_reference_type
from .transaction import is_abi_transaction_type
...
class Method:
    ...
    def __init__(
        ...
        for arg in self.args:
            if is_abi_transaction_type(arg.type):
                txn_count += 1
    ...
    def from_signature(s: str) -> "Method":
        ...
        argument_list = [
            Argument(t) for t in TupleType._parse_tuple(tokens[1])
        ]
...
class Argument:
    ...
    def __init__(
        ...
        if is_abi_transaction_type(arg_type) or is_abi_reference_type(
            arg_type
        ):
            self.type: Union[str, ABIType] = arg_type
        else:
            # If the type cannot be parsed into an ABI type, it will error
            self.type = ABIType.from_string(arg_type)
        self.name = name
        self.desc = desc
...
class Returns:
    ...
    def __init__(self, arg_type: str, desc: Optional[str] = None) -> None:
        if arg_type == "void":
            self.type: Union[str, ABIType] = self.VOID
        else:
            # If the type cannot be parsed into an ABI type, it will error.
            self.type = ABIType.from_string(arg_type)
        self.desc = desc
