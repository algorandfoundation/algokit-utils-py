...
from algokit_transact.codec._primitives import (
    decode_int_like,
    encode_bool,
    encode_int,
...
def flatten(
    child_cls: type[T], *, present_if: Callable[[Mapping[str, object]], bool] | None = None
) -> dict[str, object]:
...
def _decode_nested_field(kwargs: dict[str, object], h: _FieldHandler, payload: Mapping[str, object]) -> None:
    if h.child_cls is None or h.nested_alias is None:
        kwargs[h.name] = None
        return
    if isinstance(raw_nested := payload.get(h.nested_alias), Mapping):
        kwargs[h.name] = from_wire(h.child_cls, raw_nested)
        return
    kwargs[h.name] = None


def _decode_flatten_field(kwargs: dict[str, object], h: _FieldHandler, payload: Mapping[str, object]) -> None:
    if h.child_cls is None:
        kwargs[h.name] = None
        return
    alias_set = _wire_aliases_for(h.child_cls)
    ...
    kwargs[h.name] = from_wire(h.child_cls, sub)
