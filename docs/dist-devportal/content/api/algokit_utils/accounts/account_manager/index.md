...
### *class* EnsureFundedFromTestnetDispenserApiResult
...
#### set_signers(\*, another_account_manager: [AccountManager](#algokit_utils.accounts.account_manager.AccountManager), overwrite_existing: bool = True) → Self
...
#### set_signer_from_account(\*args: AddressWithTransactionSigner, \*\*kwargs: AddressWithTransactionSigner) → Self
...
#### get_signer(sender: str | AddressWithTransactionSigner) → TransactionSigner
...
#### get_account(sender: str) → StoredAccountType | AddressWithTransactionSigner
...
#### get_information(sender: str | AddressWithTransactionSigner) → [AccountInformation](#algokit_utils.accounts.account_manager.AccountInformation)
...
#### from_mnemonic(\*, mnemonic: str, sender: str | None = None) → AddressWithSigners
...
#### from_kmd(name: str, predicate: Callable[[dict[str, Any]], bool] | None = None, sender: str | None = None) → AddressWithSigners
...
#### logicsig(program: bytes, args: Sequence[bytes] = ()) → AddressWithSigners
...
#### multisig(metadata: MultisigMetadata, sub_signers: Sequence[AddressWithSigners]) → AddressWithSigners
...
#### localnet_dispenser() → AddressWithSigners
...
#### dispenser_from_environment() → AddressWithSigners
...
#### rekeyed(\*, sender: str, account: AddressWithTransactionSigner | AddressWithSigners) → AddressWithSigners
