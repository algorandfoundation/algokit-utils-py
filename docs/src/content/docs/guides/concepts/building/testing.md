...
The `algorand` fixture provides access to an [`AlgorandClient`](/algokit-utils-py/guides/concepts/core/algorand-client/) instance which exposes the following properties commonly used in testing:
...
- `algorand.account` - [`AccountManager`](/algokit-utils-py/guides/concepts/core/account/) for creating and managing test accounts
...
The key when generating a test account is getting hold of a [dispenser](/algokit-utils-py/guides/concepts/building/transfer/#dispenser) and then [ensuring the test account is funded](/algokit-utils-py/guides/concepts/building/transfer/#ensure_funded).
...
- `algorand.account.localnet_dispenser()` - Gets the LocalNet [dispenser](/algokit-utils-py/guides/concepts/building/transfer/#dispenser) account for funding
- `algorand.account.dispenser_from_environment()` - Gets dispenser from environment variables or LocalNet
- `algorand.account.ensure_funded(account, dispenser, min_spending_balance=...)` - [Ensures the account is funded](/algokit-utils-py/guides/concepts/building/transfer/#ensure_funded) with a minimum balance
...
When testing functionality that involves [Algorand Standard Assets (ASAs)](/algokit-utils-py/guides/concepts/building/asset/), you can create test assets using a pytest fixture or helper function. This pairs with a funded test account fixture to create ephemeral assets for each test or test suite.
...
When testing [asset transfers](/algokit-utils-py/guides/concepts/building/asset/), the receiver must first opt in to the asset before receiving it. You can then transfer assets and assert on the resulting balances.
...
When testing [box storage](/algokit-utils-py/guides/concepts/building/app-client/#boxes) operations, you need to fund the application account to cover the minimum balance requirement (MBR) for boxes, then create, write, and read boxes via the [`AppClient`](/algokit-utils-py/guides/concepts/building/app-client/#appclient). Box references must be included in the transaction so the AVM can access them.
