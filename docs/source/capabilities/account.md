...
The core internal type that holds information about a signer/sender pair for a transaction is {py:obj}`AddressWithTransactionSigner <algokit_utils.transact.AddressWithTransactionSigner>`, which represents a `TransactionSigner` (`signer`) along with a sender address (`addr`) as the encoded string address.

The following conform to `AddressWithTransactionSigner`:

- {py:obj}`AddressWithSigners <algokit_utils.transact.AddressWithSigners>` - an account that holds a private key and conforms to `AddressWithTransactionSigner`. This was previously named `SigningAccount` in v4.
- {py:obj}`LogicSigAccount <algokit_utils.transact.LogicSigAccount>` - a wrapper class around logic signature abstractions conforming to `AddressWithTransactionSigner`
- {py:obj}`MultisigAccount <algokit_utils.transact.MultisigAccount>` - a wrapper class around multisig abstractions conforming to `AddressWithTransactionSigner`
...
- {py:obj}`AddressWithSigners <algokit_utils.transact.AddressWithSigners>` - An account that holds a private key and conforms to `AddressWithTransactionSigner`. Supports rekeyed accounts.
- {py:obj}`LogicSigAccount <algokit_utils.transact.LogicSigAccount>` - An abstraction that supports logic signature signing. Exposes access to the underlying logic signature via `lsig` property.
- {py:obj}`MultisigAccount <algokit_utils.transact.MultisigAccount>` - An abstraction that supports multisig accounts with one or more signers present. Exposes access to the underlying multisig via `multisig` property.
