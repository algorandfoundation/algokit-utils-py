...
def app_client(algorand: AlgorandClient, alice: AddressAndSigner) -> ApplicationClient:
    client = ApplicationClient(algorand.client.algod, Path(__file__).parent / "app_algorand_client.json",
...
def test_add_atc(algorand: AlgorandClient, app_client: ApplicationClient, alice: AddressAndSigner):
    ...
    result = algorand.new_group().add_payment(
        PayParams(sender=alice.address, amount=0, receiver=alice.address)).add_atc(atc).execute()
    assert result.abi_results[0].return_value == DO_MATH_VALUE


def test_add_method_call(algorand: AlgorandClient, contract: Contract, alice: AddressAndSigner,
...
def test_add_method_with_txn_arg(algorand: AlgorandClient, contract: Contract, alice: AddressAndSigner,
                                 app_client: ApplicationClient):
...
def test_add_method_call_with_method_call_arg(algorand: AlgorandClient, contract: Contract, alice: AddressAndSigner,
                                              app_client: ApplicationClient):
    hello_world_call = MethodCallParams(method=contract.get_method_by_name('helloWorld'), sender=alice.address,
                                        app_id=app_client.app_id)
    result = algorand \
        .new_group() \
        .add_method_call(MethodCallParams(method=contract.get_method_by_name('methodArg'), sender=alice.address,
                                          app_id=app_client.app_id, args=[hello_world_call])) \
        .execute()
    assert result.abi_results[0].return_value == 'Hello, World!'
    assert result.abi_results[1].return_value == app_client.app_id


def test_add_method_call_with_method_call_arg_with_txn_arg(algorand: AlgorandClient, contract: Contract,
                                                           alice: AddressAndSigner, app_client: ApplicationClient):
    pay_arg = PayParams(sender=alice.address, receiver=alice.address, amount=1)
    txn_arg_call = MethodCallParams(method=contract.get_method_by_name('txnArg'), sender=alice.address,
                                    app_id=app_client.app_id, args=[pay_arg])
    result = algorand \
        .new_group() \
        .add_method_call(MethodCallParams(method=contract.get_method_by_name('nestedTxnArg'), sender=alice.address,
                                          app_id=app_client.app_id, args=[txn_arg_call])) \
        .execute()
    assert result.abi_results[0].return_value == alice.address
    assert result.abi_results[1].return_value == app_client.app_id


def test_add_method_call_with_two_method_call_args_with_txn_arg(algorand: AlgorandClient, contract: Contract,
                                                                alice: AddressAndSigner, app_client: ApplicationClient):
    pay_arg_1 = PayParams(sender=alice.address, receiver=alice.address, amount=1)
    txn_arg_call_1 = MethodCallParams(method=contract.get_method_by_name('txnArg'), sender=alice.address,
                                      app_id=app_client.app_id, args=[pay_arg_1], note=b'1')

    pay_arg_2 = PayParams(sender=alice.address, receiver=alice.address, amount=2)
    txn_arg_call_2 = MethodCallParams(method=contract.get_method_by_name('txnArg'), sender=alice.address,
                                      app_id=app_client.app_id, args=[pay_arg_2])
