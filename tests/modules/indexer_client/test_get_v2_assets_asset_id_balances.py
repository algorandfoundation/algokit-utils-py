...
from tests.fixtures.schemas.indexer import AssetBalancesResponseSchema
...
def test_basic_request_and_response_validation(indexer_client: IndexerClient, snapshot_json: SnapshotAssertion) -> None:
    ...
    result = indexer_client.lookup_asset_balances(asset_id=TEST_ASSET_ID)

    assert result.balances is not None

    validate_with_schema(result, AssetBalancesResponseSchema)
