...
Any AlgoKit Utils function that needs one of these clients will take the underlying generated clients (`algokit_algod_client.AlgodClient`, `algokit_indexer_client.IndexerClient`, `algokit_kmd_client.KmdClient`) so inline with the [Modularity](../index.md#core-principles) principle you can use existing logic to get instances of these clients without needing to use the Client management capability if you prefer.
