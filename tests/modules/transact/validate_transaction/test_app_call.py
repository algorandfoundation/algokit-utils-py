...
from tests.modules.transact._validation import assert_validation_error, build_app_call, clone_transaction
from tests.modules.transact.conftest import TestDataLookup


@pytest.fixture
def app_create_transaction(test_data_lookup: TestDataLookup) -> Transaction:
    vector = test_data_lookup("appCreate")
    return vector.transaction


@pytest.fixture
def base_update_transaction(test_data_lookup: TestDataLookup) -> Transaction:
    vector = test_data_lookup("appUpdate")
    return vector.transaction


@pytest.fixture
def base_call_transaction(test_data_lookup: TestDataLookup) -> Transaction:
    vector = test_data_lookup("appCall")
    return vector.transaction
