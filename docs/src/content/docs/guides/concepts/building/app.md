...
To get an instance of `AppManager` you can use either [`AlgorandClient`](/algokit-utils-py/guides/concepts/core/algorand-client/) via `algorand.app` or instantiate it directly (passing in an algod client instance):
...
When [sending transactions directly via AlgorandClient](/algokit-utils-py/guides/concepts/core/algorand-client/#sending-a-single-transaction) the `SendSingleTransactionResult` return value is expanded with extra fields depending on the type of app call:
...
When interacting with apps (creating, updating, deleting, calling), there are some common parameters that you will be able to pass in to all calls in addition to the [common transaction parameters](/algokit-utils-py/guides/concepts/core/algorand-client/#transaction-parameters):
